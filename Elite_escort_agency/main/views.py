...
def menuDetail(request, slug):
    ...
    return render(request, 'main/girl.html', context)
...
def add_reviews(request):
    if request.method == "POST":
        ...
        reviews.save()
        messages.success(request, "Спасибо за отзыв об этой девочке!!")
...
def add_to_cart(request, slug):
    ...
    messages.info(request, "Она тебя уже ждет! Но если хочешь, то можешь посмотреть других или заказать несколько")
...
def order_details(request):
    items = CartItems.objects.filter(user=request.user, ordered=True,status="В процессе").order_by('-ordered_date')
    cart_items = CartItems.objects.filter(user=request.user, ordered=True,status="Выполнен").order_by('-ordered_date')
...
def admin_view(request):
    cart_items = CartItems.objects.filter(item__created_by=request.user, ordered=True,status="Выполнен").order_by('-ordered_date')
    context = {
        'cart_items':cart_items,
    }
...
def update_status(request,pk):
    ...
    cart_items = CartItems.objects.filter(item__created_by=request.user, ordered=True,status="В процессе",pk=pk)
...
def pending_orders(request):
    items = CartItems.objects.filter(item__created_by=request.user, ordered=True,status="В процессе").order_by('-ordered_date')
    context = {
        'items':items,
    }
...
def admin_dashboard(request):
    ...
    pending_total = CartItems.objects.filter(item__created_by=request.user, ordered=True,status="В процессе").count()
    completed_total = CartItems.objects.filter(item__created_by=request.user, ordered=True,status="Выполнен").count()
